                                ...
                                LazyVGrid(columns: columns, spacing: UIDevice.current.userInterfaceIdiom == .phone ? 14 : 20) {
                                    if searchResults.isEmpty {
                                        ...
                                        ForEach(searchResults) { sound in
                                            SoundCell(soundId: sound.id, title: sound.title, author: sound.authorName ?? "", duration: sound.getDuration(), isNew: sound.isNew ?? false, favorites: $viewModel.favoritesKeeper, highlighted: $viewModel.highlightKeeper, nowPlaying: $viewModel.nowPlayingKeeper)
